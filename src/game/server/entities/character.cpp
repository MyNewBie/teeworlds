...
#include <game/server/gamemodes/catching.h>
...
void CCharacter::Tick()
{
	...
	if(GameServer()->m_pController->IsCatching())
	{
		...
		if(TileIndex != -1 && GameServer()->Collision()->GetCollisionCatching(TileIndex) == TILE_STOPL)
		{
			if(m_Core.m_Vel.x > 0)
			{
		...
		else if(TileIndex != -1 && GameServer()->Collision()->GetCollisionCatching(TileIndex) == TILE_STOPR)
		{
			if(m_Core.m_Vel.x < 0)
			{
				if((int)GameServer()->Collision()->GetPos(TileIndex).x > (int)m_Core.m_Pos.x)
		...
		else if(TileIndex != -1 && GameServer()->Collision()->GetCollisionCatching(TileIndex) == TILE_STOPB)
		{
			if(m_Core.m_Vel.y < 0)
			{
		...
		else if(TileIndex != -1 && GameServer()->Collision()->GetCollisionCatching(TileIndex) == TILE_STOPT)
		{
			if(m_Core.m_Vel.y > 0)
			{
				if((int)GameServer()->Collision()->GetPos(TileIndex).y < (int)m_Core.m_Pos.y)
		...
		if(g_Config.m_SvTeleport && z)
		{
			...
			if(g_Config.m_SvTeleportKill)
			{
				for(int i = 0; i < MAX_CLIENTS; i++)
				{
					...
					if(distance(pCharacter->m_Pos, m_Core.m_Pos) < ms_PhysSize)
						pCharacter->Die(GetPlayer()->GetCID(), WEAPON_WORLD);
				}
			}		
		}
