...
CCharacter *CGameWorld::IntersectCharacterTeam(vec2 Pos0, vec2 Pos1, float Radius, vec2& NewPos, CEntity *pNotThis, bool Joined)
{
	// Find other players
	float ClosestLen = distance(Pos0, Pos1) * 100.0f;
	CCharacter *pClosest = 0;
	...
	for(; p; p = (CCharacter *)p->TypeNext())
 	{
		if(p == pNotThis || Joined != p->GetPlayer()->IsJoined())
			continue;
