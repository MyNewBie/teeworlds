...
CLayerTiles::CLayerTiles(int w, int h)
{
	...
	m_Tele = 0;
	m_Speedup = 0;
	m_Color.g = 255;	m_Color.b = 255;	m_Color.a = 255;	
...
int CLayerTiles::RenderProperties(CUIRect *pToolBox)
{
	...
	if(m_pEditor->m_Map.m_pGameLayer == this || m_pEditor->m_Map.m_pTeleLayer == this || m_pEditor->m_Map.m_pSpeedupLayer == this) // remove the image and color properties if this is the game layer
	{
		aProps[3].m_pName = 0;
		aProps[4].m_pName = 0;
	}
