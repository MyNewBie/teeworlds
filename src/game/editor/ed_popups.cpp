...
static int s_GametileOpSelected = -1;int CEditor::PopupTele(CEditor *pEditor, CUIRect View)
{
	CUIRect Button;
	View.HSplitBottom(12.0f, &View, &Button);
...
int CEditor::PopupSelectGameTileOpResult()
{
	...
	static int s_aIds[NUM_PROPS] = {0};	int NewVal = 0;	int Prop = pEditor->DoProperties(&View, aProps, s_aIds, &NewVal);		if(Prop == PROP_TELE)		 pEditor->m_TeleNum = clamp(NewVal, 0, 255);		return 0;} int CEditor::PopupSpeedup(CEditor *pEditor, CUIRect View){	CUIRect Button;	View.HSplitBottom(12.0f, &View, &Button);		enum	{		PROP_FORCE=0,		PROP_ANGLE,		NUM_PROPS,	};		CProperty aProps[] = {		{"Force", pEditor->m_SpeedupForce, PROPTYPE_INT_SCROLL, 0, 255},		{"Angle", pEditor->m_SpeedupAngle, PROPTYPE_INT_SCROLL, 0, 359},		{0},	}; 	static int s_aIds[NUM_PROPS] = {0};	int NewVal = 0;	int Prop = pEditor->DoProperties(&View, aProps, s_aIds, &NewVal);		if(Prop == PROP_FORCE)		pEditor->m_SpeedupForce = clamp(NewVal, 0, 255);	if(Prop == PROP_ANGLE)		pEditor->m_SpeedupAngle = clamp(NewVal, 0, 359);		return 0;}